...
plus a digest of the redline page model (positions, not just counts).
...
from parity.common import CORPUS, page_digest, pairs  # noqa: E402
...
def main() -> int:
    ...
    for alias, v in sorted(golden.items()):
        print(f"{alias:12s} a={v['a']:4d} b={v['b']:4d} redline={v['redline']:4d} {v['digest']}")
    print(f"written: {OUT}")
    return 0
